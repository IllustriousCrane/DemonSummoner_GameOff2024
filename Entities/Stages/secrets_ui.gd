...
func add_secrets():
	...
	for i in Globals.active_secrets.size():
...
func show_info(secret):
	
	$SecretInfo.show()
	$SecretInfo/Control/tagline.text = Globals.Secrets[secret].secret_name
	$SecretInfo/Control/description.text = Globals.Secrets[secret].secret_description
